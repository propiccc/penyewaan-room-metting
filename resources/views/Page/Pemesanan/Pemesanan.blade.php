            ...
            @if (count($pemesanan) != 0) 
                @foreach ($pemesanan as $item )
                    <div class="w-[350px] p-6 flex flex-col bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{$item->Room->room_name}}</h5>
                        <div class="h-[2px] bg-black mb-3"></div>
                        <span class="mb-3 text-black font-semibold dark:text-gray-400">Rp {{$item->Room->price}}</span>
